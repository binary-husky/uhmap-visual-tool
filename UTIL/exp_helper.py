...
class MySFTPClient(paramiko.SFTPClient):
    ...
    def get_dir(self, source, target, ignore_list=[], win_compat=False):
        ...
        for item in self.listdir(source):
            ...
            if self.isfile(os.path.join(source, item).replace('\\','/')):
                # print亮靛('uploading: %s --> %s'%(os.path.join(source, item),'%s/%s' % (target, item)))
                dst =  '%s/%s' % (target, item)
                if win_compat: dst = dst.replace(':', '_')
                self.get(os.path.join(source, item).replace('\\','/'), dst)
            else:
                ...
                if win_compat: dst = dst.replace(':', '_')
                os.mkdir(dst)
                self.get_dir(os.path.join(source, item).replace('\\','/'), dst, ignore_list, win_compat)
